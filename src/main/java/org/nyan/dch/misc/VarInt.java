...
import static org.nyan.dch.misc.BitcoinUtils.isLessThanUnsigned;
...
public class VarInt {
    ...
    public VarInt(byte[] buf, int offset) {
        ...
        if (first < 253) {
            ...
            // 32 bits.
            val = BitcoinUtils.readUint32(buf, offset + 1);
            originallyEncodedSize = 5;
        } else {
            // 64 bits.
            val = BitcoinUtils.readUint32(buf, offset + 1) | (BitcoinUtils.readUint32(buf, offset + 5) << 32);
    ...
    public byte[] encode() {
        if (isLessThanUnsigned(value, 253)) {
            ...
            bytes[0] = (byte) 254;
            BitcoinUtils.uint32ToByteArrayLE(value, bytes, 1);
            return bytes;
        } else {
            byte[] bytes = new byte[9];
            bytes[0] = (byte) 255;
            BitcoinUtils.uint32ToByteArrayLE(value, bytes, 1);
            BitcoinUtils.uint32ToByteArrayLE(value >>> 32, bytes, 5);
            return bytes;
        }
    }
}
